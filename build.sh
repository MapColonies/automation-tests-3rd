...
docker build --no-cache -t 3rd-automation-test:$VERSION --build-arg VERSION=$VERSION .
#docker build -t automation-test:$VERSION --build-arg VERSION=$VERSION .

docker tag 3rd-automation-test:$VERSION 3rd-automation-test:latest

echo automation-test:$VERSION > ./docker_images/generated_dockers.txt


# path build.sh with "DUMP_IMAGE=1" to save docker
FILE_NAME=3rd-automation-test:${VERSION}.tar
